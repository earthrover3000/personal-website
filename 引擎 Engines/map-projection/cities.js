// Shared city reference data for the map pages.
//
// SINGLE SOURCE OF TRUTH for city coordinates, consumed by TWO pages:
//   • Urbanism "City Map Explorer" (cities-transport) → window.URBANISM_DATA
//     { gawcList, livedList } — the dropdown location lists it swaps between.
//   • Maps & Projections "Great Circle Mapper" → window.WORLD_CITIES
//     { world, lived } — the two mutually-exclusive city-dot layers.
//
// Lives in the engine module folder (引擎 Engines/map-projection) so it deploys
// automatically with the rest of the map data and both pages load the same file
// (no duplicated coordinates). Both pages <script src> it before their init.
//
// All coords are the lat/lon of each city's OSM place=city node, fetched via
// Overpass with Wikidata QIDs unioned into one request:
//   [out:json];(node["wikidata"="<Q>"]["place"~"city|town"]; ...);out;
// Inline `// node N — Tier` comments record the OSM node ID (so each row is
// verifiable at openstreetmap.org/node/<N>) and the GaWC tier.
...
(function () {
  ...
  // consumers expect. '---' becomes a visual divider option (dropdowns only).
  function pick(slugs) {
  ...
  // The cities the site owner has lived in (front-loaded into the Urbanism
  // "lived-in" list; the Great Circle Mapper offers them as their own layer).
  var LIVED_SLUGS = ['dublin', 'cambridge', 'london', 'shanghai'];
  ...
  // Urbanism City Map Explorer — dropdown lists (keep the '---' dividers).
  window.URBANISM_DATA = {
    gawcList: pick(GAWC_LIST),
    livedList: pick(LIVED_LIST)
  };

  // Great Circle Mapper — two mutually-exclusive city-dot layers. Divider-free:
  // `world` = the 49 GaWC cities; `lived` = the 4 the owner has lived in.
  window.WORLD_CITIES = {
    world: pick(GAWC_LIST.filter(function (s) { return s !== '---'; })),
    lived: pick(LIVED_SLUGS)
  };
})();
